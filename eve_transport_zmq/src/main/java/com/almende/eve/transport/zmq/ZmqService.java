...
public class ZmqService implements TransportService {
	 ...
	 *            {Integer} basePort
	 */
	public ZmqService(final Map<String, Object> params) {
		if (params != null) {
	...
	public String getAgentId(final URI agentUrl) {
		for (final Entry<String, ZmqConnection> entry : inboundSockets
				.entrySet()) {
	...
	private URI genUrl(final String agentId) {
		...
		if (baseUrl.startsWith("tcp://")) {
			final int basePort = Integer
					.parseInt(baseUrl.replaceAll(".*:", ""));

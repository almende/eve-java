...
public class WeightsUtil {
	 ...
	 */
	public static void order(final List<Weight> intervals) {
	...
	public static List<Weight> merge(final List<Weight> intervals) {
		// copy the intervals
		final ArrayList<Weight> merged = new ArrayList<Weight>();
		
		for (final Weight newInterval : intervals) {
			...
			while (i < merged.size() && interval != null) {
				final Weight other = merged.get(i);
				if (interval.getInterval().overlaps(other.getInterval())) {
					...
					// find the start of the center interval (= end of left
					// interval)
					...
					final Double centerWeight = interval.getWeight()
							+ other.getWeight();
					
					// find the end of the center interval (= start of right
					// interval)
					...
					if (centerEnd.isAfter(centerStart)) {
						final Weight centerInterval = new Weight(centerStart,
								centerEnd, centerWeight);
						merged.set(i, centerInterval);
					}
					
					// insert a new interval left from the overlapping part
					if (centerStart.isAfter(start)) {
						final Weight leftInterval = new Weight(start,
								centerStart, leftWeight);
					...
					if (end.isAfter(centerEnd)) {
						interval = new Weight(centerEnd, end, rightWeight);
					} else {
						interval = null;
					}
		...
		int i = 1;
		while (i < merged.size()) {
			final Weight prev = merged.get(i - 1);
			final Weight cur = merged.get(i);
			if (prev.getWeight().equals(cur.getWeight())
					&& prev.getEnd().equals(cur.getStart())) {
				final Weight combi = new Weight(prev.getStart(), cur.getEnd(),
						prev.getWeight());
	...
	public static void main(final String[] args) {
		...
		intervals.add(new Weight(new DateTime(2012, 8, 4, 8, 0, 0),
				new DateTime(2012, 8, 4, 10, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 12, 0, 0),
				new DateTime(2012, 8, 4, 14, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 13, 0, 0),
				new DateTime(2012, 8, 4, 16, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 3, 0, 0),
				new DateTime(2012, 8, 4, 4, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 3, 0, 0),
				new DateTime(2012, 8, 4, 4, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 3, 0, 0),
				new DateTime(2012, 8, 4, 5, 0, 0), new Double(1)));
		
		intervals.add(new Weight(new DateTime(2012, 8, 4, 17, 30, 0),
				new DateTime(2012, 8, 4, 18, 0, 0), new Double(1.5)));
		intervals.add(new Weight(new DateTime(2012, 8, 4, 17, 0, 0),
				new DateTime(2012, 8, 4, 17, 30, 0), new Double(1.5)));
		
		final List<Weight> merged = WeightsUtil.merge(intervals);
		for (final Weight i : merged) {

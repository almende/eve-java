 ...
 * 
 * @brief
 *        GoogleTranslateAgent can translate text, using the Google Translator
 *        API.
 * 
 * @license
...
public class GoogleTranslateAgent extends Agent {
	...
	public String translate(@Name("text") final String text,
			...
			@Name("source") @Optional final String source) throws Exception {
		final String key = getState().get("key", String.class);
		if (key == null) {
		...
		String url = TRANSLATE_API_URL + "?q="
				+ URLEncoder.encode(text, "UTF-8") + "&key="
				+ URLEncoder.encode(key, "UTF-8") + "&target="
